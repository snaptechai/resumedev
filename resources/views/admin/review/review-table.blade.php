                ...
                <tr class="hover:bg-gray-50 transition">
                    ...
                    @if ($status == 0)
                        <td class="px-6 py-4 text-sm text-gray-500">
                            <select class="status-dropdown border border-gray-300 rounded px-2 py-1"
                                ...
                                <option value="1" {{ $review->status == 'Approved' ? 'selected' : '' }}>Approved</option>
                    ...
                    <td class="px-6 py-4 text-sm text-gray-500">
                        <div class="flex items-center justify-center">  
                            <form action="{{ route('review.destroy', $review->id) }}" method="POST" onsubmit="return confirm('Are you sure you need to Delete this Review ?');">
                                @csrf @method('DELETE')
