<x-layouts.app>
                ...
                <div class="px-4 py-3 border-b flex justify-between items-center mb-2">
                    ...
                    <div class="flex items-center"> 
                        <div class="relative inline-block text-left">
                            <button type="button" onclick="openModal('create-link')" class="inline-flex items-center px-3 py-1.5 text-sm border border-gray-300 text-gray-700 rounded-md hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500" id="actions-menu-button" aria-expanded="false" aria-haspopup="true"> Add New Link +
                            </button> 
                        </div>
                    </div> 
                </div>
                    ...
                    <table class="w-full divide-y divide-gray-200">
                        <thead>
                            <tr class="bg-gray-50">
                                ...
                                <th scope="col" class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Original Url</th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">New Url</th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Note</th>
                            ...
                            @foreach ($redirect_links as $index => $val)
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    ...
                                    <td class="px-6 py-5 whitespace-nowrap">
                                        <div class="flex items-center"> 
                                            <div class="ml-4">
                                                <div class="text-sm font-medium text-gray-900">{{ $val->original_url }}</div> 
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-5 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-5 whitespace-nowrap">
                                        <div class="flex items-center"> 
                                            <div class="ml-4">
                                                <div class="text-sm font-medium text-gray-900">{{ $val->added_date }}</div> 
                                            </div>
                                        </div>
                                    </td>
                                    <td class="flex px-6 py-5 whitespace-nowrap text-sm font-medium text-center my-3">
                                        <div class="flex space-x-3">
                                            <button type="button" onclick="openModal('edit-{{ $val->id }}')" class="group flex items-center justify-center h-8 w-8 rounded-full bg-green-50 text-green-600 hover:bg-green-100 transition-colors duration-150" title="Edit">
                                        ...
                                        <div class="flex space-x-3">  
                                            <form action="{{ route('redirectlink.destroy', $val->id) }}" method="POST" onsubmit="return confirm('Are you sure you need to Delete this Coupon?');">
                                                @csrf @method('DELETE')
                                ...
                                @include('admin.redirect-link.edit', ['val' => $val])
                            @endforeach
                        </tbody>
                    </table>
                </div>
    ...
    @include('admin.redirect-link.create')
</x-layouts.app>
