<form action="{{ route('faqs.update', $faq->id) }}" method="POST">
    ...
    <div class="p-6 space-y-4">
        <div>
            <label for="edit-question-{{ $faq->id }}" class="block text-sm font-medium text-gray-700 mb-2">
                Question
            </label>
            <input type="text" id="edit-question-{{ $faq->id }}" name="question"
        ...
        <div>
            <label for="edit-answer-{{ $faq->id }}" class="block text-sm font-medium text-gray-700 mb-2">
                Answer
            </label>
            <textarea id="edit-answer-{{ $faq->id }}" name="answer" rows="4" required
                class="w-full rounded-lg border-gray-300 py-3 px-3 text-gray-700 focus:ring-2 focus:ring-[#BCEC88] focus:border-[#BCEC88] appearance-none border">{{ old('answer', $faq->answer) }}</textarea>
        </div>
    </div>
